...
export const provisionConfigCheck = refine(object_({
  ...
  label: text('lumiapp'),
  target: object_({ host: hostname_(), initialUser: withDefault(unixUser_(), 'root'), initialAuth: object_({ mode: withDefault(oneOf(['key','password','auto']), 'auto') }), initialSshPort: withDefault(port_(), 22), finalSshPort: withDefault(port_(), 5564), adminUser: withDefault(unixUser_(), 'lumiadmin'), deployUser: withDefault(unixUser_(), 'lumideploy'), hostnameOverride: maybe(hostname_()), knownHostsPath: maybe(string_({ minLength: 1 })), identityPath: maybe(string_({ minLength: 1 })), keyDirectory: text('.secrets/keys') }),
