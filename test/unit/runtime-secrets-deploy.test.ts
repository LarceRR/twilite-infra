...
test('Deployment requires immutable release and separate migration/readiness/smoke gates', () => { match(deploy, /API_IMAGE_DIGEST.*sha256/); match(deploy, /docker pull/); match(deploy, /\/app\/bin\/migrate/); match(deploy, /Health\.Status.*healthy/); match(deploy, /curl --fail/); match(deploy, /previous\.digest/); match(rollback, /previous\.digest/); strictEqual(deploy.includes('latest'), false); });
