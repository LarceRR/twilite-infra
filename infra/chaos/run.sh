...
case "$CHAOS_SCENARIO" in
  ...
  vm-reboot) bash "$CHAOS_REPO_ROOT/src/harness/vmctl/main.ts" reboot ;;
  ...
  broken-release) MIGRATION_FAILURE=1 bash "$CHAOS_REPO_ROOT/infra/deploy/deploy.sh" || status=$? ;;
  migration-failure) MIGRATION_FAILURE=1 bash "$CHAOS_REPO_ROOT/infra/deploy/deploy.sh" || status=$? ;;
  ...
  partial-rerun) bash "$CHAOS_REPO_ROOT/infra/chaos/partial-rerun.sh" ;;
  vps-loss) bash "$CHAOS_REPO_ROOT/src/harness/vmctl/main.ts" destroy ;;
