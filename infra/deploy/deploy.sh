...
# 1. Resolve immutable release and 2. verify integrity.
...
# Keep the previous known-good release before 5. migration.
if [[ -f "$CURRENT_FILE" ]]; then
  cp --preserve=mode "$CURRENT_FILE" "$PREVIOUS_FILE"
fi

# 5. Run DB migration as a separate, fixed contract. Expand/contract only.
"${compose[@]}" run --rm --no-deps --entrypoint /app/bin/migrate api

# 6. Verify migration success is represented by the command exit code.
"${compose[@]}" config --quiet

# 7. Start the new immutable version.
...
# 9. Smoke gate. Staging credentials are injected through the protected environment, never echoed.
curl --fail --silent --show-error --max-time 15 "$SMOKE_URL" >/dev/null

# 10-11. Traffic continues through the healthy container; record success.
