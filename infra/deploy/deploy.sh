...
[[ -d "$DEPLOY_ROOT/infra/compose" ]] || { echo "deployment contract is missing at $DEPLOY_ROOT" >&2; exit 1; }

case "$DEPLOY_ENV" in
  production) PROJECT_NAME="${PROD_PROJECT_NAME:-lumi-prod}"; COMPOSE_ENV="$DEPLOY_ROOT/infra/compose/production/compose.yaml" ;;
  staging) PROJECT_NAME="${STAGE_PROJECT_NAME:-lumi-staging}"; COMPOSE_ENV="$DEPLOY_ROOT/infra/compose/staging/compose.yaml" ;;
esac
